...
- 2026-06-23：T105 R010 依赖门禁已归入 StateEngine。`WorkflowService` 的 generate/edit/approve/retry/final_delivery 前置依赖统一调用 `StateEngine.assert_can_generate()`；依赖不满足继续返回 `UPSTREAM_NOT_APPROVED`，同时写 `state_transition_log.trigger=dependency_gate_blocked`，reason 包含 `rule_id=R010`、`handled_by=StateEngine` 和未通过上游状态。`RuleExecutor` 不再执行 `dependency_check/dependency_passable`，只在 `/rules/coverage` 中把 R010 标记为 `handled_by=StateEngine`；验证 `python -m pytest apps\api\tests -q` 为 `201 passed, 2 xfailed`。
- 2026-06-23：T104 StateEngine Phase 1 已完成。`StateEngine.transition()` 成为 approve、cascade invalidate、config skip/restore 的统一状态迁移入口；generate/edit 通过 `record_version_ready()` 记录 `not_started|approved|blocked -> drafted -> needs_review` 两段迁移；`/retry` 不再简单转发 generate，而是先记录 `user_redo` 到 `drafted` 再生成。R010 依赖门禁职责从 `RuleExecutor.record_r010_result()` 迁到 StateEngine `state_transition_log` 的 `dependency_gate_blocked` 诊断日志，`rule_result_log` 不再写 R010 运行结果。验证：`python -m pytest apps/api/tests -q` 为 `201 passed, 2 xfailed`。后续风险：`VideoOrchestrator` 和 provider 失败路径仍有少量直接 `store.write_version(..., blocked|needs_review|drafted)`，Phase 2 应继续收口 blocked/async task 状态写入，避免绕过 StateEngine。
- 2026-06-23：T109 StateEngine Phase 2 已完成。`RuleExecutor` hard_block 不再直改 `node_state`，由 `WorkflowService._run_rules_for_event()` 调用 `StateEngine.mark_blocked()`；provider failed、`intro_video_asset` 部分失败、`VideoOrchestrator.final_video` drafted/blocked/needs_review 均改为“先写版本、同分支立即 `StateEngine.record_written_version()` 写 transition”。新增 workflow 合法触发 `provider_failed`、`final_video_compose_failed`、`video_tasks_submitted`。验证：Phase2 `3 passed`，状态/规则组合 `36 passed`，视频/provider 组合 `82 passed`，后端全量 `204 passed, 2 xfailed`。剩余直写只允许作为 `store.py` 底层 primitive、`state_engine.py` 入口或服务/视频编排中已由 StateEngine helper 包裹的版本创建。
- 2026-06-23：T129 工作区用户态契约已完成。新增 `GET /projects/{project_id}/workspace`，前端无需解析 raw manifest 即可拿到 7 个用户态步骤：项目信息、教材内容、教案生成、导入视频方案、PPT 草稿、视频生成、最终交付。主步骤字段只返回 `step_id/title/state/goal/user_action/result/primary_action/lock_reason/review` 等用户态信息；`node_id/schema/depends_on/rule_summary/latest_transition/review_reason/artifact` 统一隔离到 `developer_diagnostics` 折叠区。锁定文案转为“请先确认【教材内容】后，再进入【教案生成】。”这类行动建议，不暴露 R010、StateEngine、manifest。验证：T129 目标测试 `3 passed`，T129+状态/PPT/API 相关回归 `39 passed`；后端全量当前仍有 3 个非 T129 失败，分别属于 DeepSeek 旧断言与 T127/T128 教材资产包。
- 2026-06-23：T127 教材目录解析已完成。人教版一年级上册教材库列表返回中文展示字段 `publisher/subject_label/grade_label/volume_label/display_name` 和 `status=indexed`；知识点接口返回 7 个目录章节（数学游戏、5以内数的认识和加减法、6~10的认识和加减法、认识立体图形、11~20的认识、20以内的进位加法、复习与关联）、教材页码/PDF 页码映射，以及带 `chapter_id/page_start/page_end/pdf_page_start/pdf_page_end/keywords/parse_status/review_status` 的课时知识点。目录章节不伪装成课时知识点；未知知识点稳定返回 `TEXTBOOK_ASSET_NOT_FOUND`。验证：`python -m pytest apps\api\tests\test_textbook_pdf_parsing.py -q` 为 `13 passed`，后端全量 `python -m pytest apps\api\tests -q` 为 `221 passed, 2 xfailed`。剩余风险：目录数据仍是固定 fixture 常量，不承诺任意教材泛化；真实目录 OCR/MinerU 自动识别留给后续任务。
- 2026-06-23：T128 教材证据包可信化已完成。`TextbookParser._write_pdf_slice()` 不再在裁剪失败时复制整册 PDF；`TextbookLibraryStore.extract_asset()` 只处理当前 `knowledge_point_id`，失败时落库 failed 资产、保留页码范围和 `diagnostics`，并通过 API 返回 `TEXTBOOK_ASSET_EXTRACT_FAILED`。非 `kp_001` 的 fixture Markdown 也改为教材结构化契约章节，不再输出纯“待 MinerU 精抽”占位。`confirm_asset()` 现在确认前校验页段 PDF 页数小于整册、Markdown 契约章节、checksum 和 pending 状态，不可信返回 `TEXTBOOK_ASSET_NOT_TRUSTED`。验证：`python -m pytest apps\api\tests\test_textbook_pdf_parsing.py -q` 为 `13 passed`；后端全量 `python -m pytest apps\api\tests -q` 为 `221 passed, 2 xfailed`。
- 2026-06-24：V0.6 并行冻结基线。当前阶段若前端/测试正在做用户态红线复测或封板，后端不得并行修改被测试锁定的接口字段、`/workspace` 用户态契约、教材三段式 API、资产状态、provider/storage 行为或 StateEngine/RuleExecutor 状态语义。允许并行的后端任务仅限不影响验收基线的文档、Runbook、接口方案、只读调研或隔离 spike；若必须修改冻结区，需先交由首席系统架构师重新排期并通知测试重建基线。
- 2026-06-24：T006 后端 1 已补齐 API 运行契约文档 `docs\backend-runtime-contract.md`，仅做文档，不实现业务功能、不读取真实密钥、不改任务状态。契约确认 API 必须从仓库根目录启动，推荐 Python 3.11，依赖入口为 `apps\api\requirements.txt`；`/health` 只证明进程和 workflow 版本可用，不证明 storage/provider/鉴权 readiness；普通 API 未配置 `BACKEND_API_TOKEN` 时仍为本地/内测放行，admin 未配置 token 时隐藏为 404，生产必须补正式鉴权或至少强制 token；storage/SQLite 当前只适合单机单 API 实例，T008 容器化必须挂载持久卷并默认单实例；真实文本、图片、视频、TTS provider 均需服务端密钥注入，长任务当前依赖 task 查询推进，生产前需补超时、退避重试、队列、监控和备份恢复。T009 测试计划应区分 `/health` liveness、storage readiness、fake/placeholder E2E、真实 provider 单项 smoke、ffmpeg 合成失败和防误提交。
- 2026-06-24：T144 已补 `/workspace` 用户态契约聚合。7 步主导航不变，但步骤新增 `current_action`、`review_summary`、`sub_gates`；PPT 草稿子门禁聚合结构方案/逐页脚本/视觉资产/PPTX 文件，视频生成子门禁聚合文稿/分场剧本/资产与首帧/分镜/clip-TTS-合成。历史命名兼容只放 `sub_gates[].compatibility` 和 `developer_diagnostics.compatibility_aliases`：`lesson-plan-final -> lesson_plan`、`video-screenplay -> intro_video_screenplay`、`pptx-generation -> pptx_artifact`、`video-generation/video_clip_generation -> final_video`。本轮未接真实 provider，验证 `python -m pytest apps\api\tests\test_workspace_user_flow_contract.py -q` 为 `6 passed`。
- 2026-06-24：T144 质量修复后确定 `/workspace` 普通用户态 `sub_gates[]` 不得暴露 `compatibility`、历史 node 别名或工程命名；普通子门禁只保留 `gate_id/title/state/status_text/review_summary`。`lesson-plan-final`、`video-screenplay`、`pptx-generation`、`video-generation/video_clip_generation` 等兼容别名只能放入 `developer_diagnostics.steps.*.compatibility_aliases`，并由前端默认折叠。验证：`python -m pytest apps\api\tests\test_workspace_user_flow_contract.py -q` 为 `6 passed`，workspace/PPT/video 组合回归为 `26 passed`。
- 2026-06-24：T152-T154 后端链路已通过 T157 封板复核。教案库支持上传 PDF/Word/Markdown/Markdown 文本到全局教案库；创建项目携带 `reference_lesson_plan_id` 后不强制教材绑定，可直接从引用教案生成 `lesson_plan=needs_review`。direct lesson workspace 用户态中“教材内容”显示由教案导入替代并跳过，底层诊断保留 `textbook_parse=skipped`，当前步骤为 `lesson_plan` 且主操作可用。管理教材库所需教材库、知识点、切分、解析、确认资产和教案库接口已供管理员页调用。新鲜验证：`python -m pytest apps\api\tests\test_lesson_plan_library_upload.py apps\api\tests\test_workspace_user_flow_contract.py apps\api\tests\test_direct_lesson_project_contract.py -q` 为 `12 passed`，`python -m pytest apps\api\tests\test_textbook_pdf_parsing.py -q` 为 `19 passed`。该结论不代表生产权限、真实 MinerU provider 或管理员批量写入全流程通过。
