...
- 2026-06-29：视频工作台 Staging Smoke 与回滚演练交付门禁完成。新增 `apps/api/scripts/video_workbench_staging_smoke.py`、脚本单测、前端 staging smoke 契约、staging runbook、rollback drill 和本地 fake smoke 脱敏报告。验证：新增契约先红后转绿；脚本单测 `3 passed`；前端视频工作台契约集合通过；本地 fake smoke 通过并生成 `docs/qa-audits/2026-06-29-145615-local-fake-video-workbench-staging-smoke.*`；后端目标 `50 passed, 2 xfailed`；`tsc/lint/build/scan:client-secrets`、正式入口 Playwright `3 passed`。剩余风险：尚未在 staging HTTP 环境和真实 Omni provider 上执行。
- 2026-06-29：视频工作台 P2-1 素材 DB 元数据副本验收通过。项目 SQLite 新增 `video_workflow_assets`；上传参考图写入 DB 元数据，删除同步 `deleted_at`，storage cleanup 同步 `purged_at`。验证：新增测试先红 `no such table: video_workflow_assets` 后转绿；后端目标 `50 passed, 2 xfailed`，前端契约/类型、lint、build、客户端密钥扫描通过。当前仍保留 `assets.json` 兼容读取路径，不等同于完整 DB 化迁移。
- 2026-06-29：视频工作台 P2-2 存储生命周期运维入口验收通过。新增 `GET /projects/{project_id}/video-workflow/storage` 和 `POST /projects/{project_id}/video-workflow/storage/cleanup`；后端契约覆盖正式 API 查询策略/用量、cleanup 清理过期软删除参考图、保留 manifest 历史元数据、物理文件删除和用量归零。验证：新增测试先红 404 后转绿；后端目标 `49 passed, 2 xfailed`，前端契约/类型、lint、build、客户端密钥扫描通过。当前仍不是定时清理任务或对象存储生命周期策略。
- 2026-06-29：视频工作台 P2-5 部署拓扑契约封板完成。`video-workbench-ci-contract.test.ts` 现在读取 `docs\ops-video-workbench-deployment-topology.md`，锁定 Browser -> Next `/api/backend` -> FastAPI 拓扑、观测接口代理路径和部署环境 E2E 验收口径。验证：契约先红缺 observability 路径，补文档后前端契约/类型、lint、build、客户端密钥扫描和 `git diff --check` 均通过。剩余风险是真实 staging/production 部署 E2E 尚未执行。
- 2026-06-29：视频工作台 P2-3 可观测性接口化验收通过。新增正式后端接口 `GET /projects/{project_id}/video-workflow/observability`，测试覆盖按项目返回 metrics/events、事件包含 `project_id/run_id/trace_id`，并验证不泄露 provider raw、签名 URL、临时 token、授权头字段。验证：新增测试先红 404 后转绿；后端目标 `48 passed, 2 xfailed`，前端契约/类型、lint、build、客户端密钥扫描和 `git diff --check` 均通过。当前仍不是外部监控系统接入，事件仍是 API 进程内 ring buffer。
- 2026-06-29：视频工作台正式入口浏览器验收补证通过。`bunx playwright test e2e/video-workbench.spec.ts --workers=1` 得到 `3 passed`，覆盖正式项目工作区入口无需打开开发诊断、上传参考图并限制 7 张、生成前确认弹窗包含模型和参考图数量、双击生成只创建 1 个任务、参考图顺序传给 API、完成后真实小 MP4 触发 `loadedmetadata` 且 `duration > 0`、刷新后历史视频可见、复用参数、跨标签同步、离线暂停/恢复轮询。已将 `apps/web/test-results/` 和 `apps/web/playwright-report/` 加入 `.gitignore`，避免 Playwright 产物误提交。仍未覆盖真实 provider 付费任务和触屏物理设备手势。
- 2026-06-29：视频工作台 P1-7 草稿、项目切换与费用保护代码级验收通过。既有能力已覆盖按项目 localStorage 草稿、`key={projectId}` remount 清旧项目临时状态、前端/Store 创建锁、服务端单项目活动任务限制、`submission_unknown` 重复费用确认；本轮新增服务端项目/全局创建窗口限流，`VIDEO_RATE_LIMITED` 以 429/retryable/action=wait_and_retry 返回，前端生成前确认模型/尺寸/时长/参考图数量/频率限制。验证：`python -m pytest apps/api/tests/test_video_workflow_canvas.py -q` 得到 `41 passed`；`bun src/lib/video-workflow-contract.test.ts`、`bun src/lib/video-workflow-errors.test.ts`、`bunx tsc --noEmit --incremental false`、`bun run lint`、`bun run build`、`bun run scan:client-secrets`、`git diff --check` 通过。仍需补真实浏览器确认弹窗和项目切换 DOM 证据。
- 2026-06-29：视频工作台 P1-6 下载与播放器代码级验收通过。后端覆盖 Octo 下载 Content-Type 白名单、无效 Content-Length、Content-Length/流式超大小限制、MP4 `ftyp` 校验、原子 `.part` 下载失败清理、`download_bytes/download_sha256` 持久化；前端覆盖播放器加载失败、重新加载、下载源文件入口契约。验证命令：`python -m pytest apps/api/tests/test_video_workflow_canvas.py -q` 得到 `40 passed`；Octo 下载目标用例 `4 passed`；`bun src/lib/video-workflow-contract.test.ts`、`bun src/lib/video-workflow-errors.test.ts`、`bun src/lib/video-workflow-polling-controller.test.ts`、`bun src/lib/video-workflow-upload-queue.test.ts`、`bunx tsc --noEmit --incremental false`、`bun run lint`、`bun run build`、`bun run scan:client-secrets`、`git diff --check` 均通过。该结论仍不是真实浏览器 `loadedmetadata`/解码失败 E2E，后续需补 Playwright 小 MP4 证据。
- 2026-06-29：视频工作台 P1-5 上传交互代码级验收通过。覆盖单文件状态队列、同名文件状态隔离、超大文件本地失败、失败文件保留原始 `File` 以便重试、组件契约中“重试”和“部分参考图已上传”文案。验证命令：`bun src/lib/video-workflow-upload-queue.test.ts`、`bun src/lib/video-workflow-contract.test.ts`、`bun src/lib/video-workflow-errors.test.ts`、`bun src/lib/video-workflow-polling-controller.test.ts`、后端上传边界 `python -m pytest apps/api/tests/test_video_workflow_canvas.py::test_video_workflow_partial_upload_keeps_valid_files apps/api/tests/test_video_workflow_canvas.py::test_video_workflow_rejects_non_image_reference_upload -q`、`bunx tsc --noEmit --incremental false`、`bun run lint`、`bun run build`、`bun run scan:client-secrets`、`git diff --check` 均通过。该结论仍不是浏览器触屏验收，后续 P1-5 可补 Playwright/真实 DOM 证据。
- 2026-06-29：视频工作台 P1-4 错误提示分层代码级验收通过。本轮覆盖后端 envelope action、前端结构化错误 formatter、toast 脱敏、轮询 `{ok:false}` 退避，目标测试为后端 `44 passed, 2 xfailed`、前端 `video-workflow-errors/contract/polling-controller`、`api-proxy/api-warning` 契约、`tsc`、`lint`、`build`、`scan:client-secrets`、`git diff --check`。尚未补真实浏览器错误态截图和触屏上传交互专项；P1-5/P1-6/P1-7、P2 仍需后续独立验收。
